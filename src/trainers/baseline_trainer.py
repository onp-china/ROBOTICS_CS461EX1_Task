...
from src.models.diffusion_transformer_policy import BaselineDiffusionTransformerPolicy, AttnResDiffusionTransformerPolicy
...
def build_model(config: Dict) -> BaselineDiffusionTransformerPolicy:
    model_type = config.get("model", {}).get("type", "baseline")
    kwargs = dict(
    ...
    if model_type == "attnres":
        return AttnResDiffusionTransformerPolicy(**kwargs)
    return BaselineDiffusionTransformerPolicy(**kwargs)
