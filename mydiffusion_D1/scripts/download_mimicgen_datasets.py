...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Download the three MimicGen D1 datasets via the official downloader.")
    parser.add_argument(
        "--mimicgen-repo",
        default=None,
        help="Path to a local MimicGen checkout. Falls back to MYDIFFUSION_D1_MIMICGEN_REPO_ROOT.",
    )
    parser.add_argument(
        "--python",
...
def normalize_download_layout(download_root: Path) -> None:
    ...
    if found != expected:
        fail(
            "Expected `mydiffusion_D1/data/mimicgen/raw/core/` to contain exactly the three target "
