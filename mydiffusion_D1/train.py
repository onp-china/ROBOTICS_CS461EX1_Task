...
MYDIFFUSION_D1_ROOT = Path(__file__).resolve().parent
if str(MYDIFFUSION_D1_ROOT) not in sys.path:
    sys.path.insert(0, str(MYDIFFUSION_D1_ROOT))

from _runtime import DIFFUSION_POLICY_ROOT, add_repo_paths, fail, register_mimicgen_environments, require_modules
    ...
    if normalized.startswith("cuda") and not torch.cuda.is_available():
        ...
        if has_mps:
            fail(
                ...
                "Example: `python mydiffusion_D1/train.py "
                "--config-name=train_diffusion_transformer_mimicgen_d1_lowdim_abs_workspace "
                "task=mug_cleanup_d1_lowdim_abs training.seed=42 training.device=mps`."
            )
        fail(
            f"Configured `training.device={device_name}`, but CUDA is unavailable in this environment. "
...
def main() -> None:
    ...
    @hydra.main(
        version_base=None,
        config_path=str(DIFFUSION_POLICY_ROOT / "config"),
        config_name="train_diffusion_transformer_mimicgen_d1_lowdim_abs_workspace",
    )
    def _main(cfg: OmegaConf) -> None:
        ...
        if not manifest_ready:
            fail(
                ...
                "`python mydiffusion_D1/scripts/inspect_mimicgen_dataset.py` first so the "
        ...
        if not dataset_path.is_file():
            fail(
                ...
                "Run `python mydiffusion_D1/scripts/convert_mimicgen_abs_actions.py` after "
