...
MYDIFFUSION_D1_ROOT = REPO_ROOT / "mydiffusion_D1"
DIFFUSION_POLICY_ROOT = MYDIFFUSION_D1_ROOT / "diffusion_policy"

MIMICGEN_REPO_ENV_VAR = "MYDIFFUSION_D1_MIMICGEN_REPO_ROOT"
MYDIFFUSION_D1_DATA_DIR_ENV_VAR = "MYDIFFUSION_D1_DATA_DIR"
MYDIFFUSION_D1_OUTPUTS_DIR_ENV_VAR = "MYDIFFUSION_D1_OUTPUTS_DIR"
MYDIFFUSION_D1_REPORTS_DIR_ENV_VAR = "MYDIFFUSION_D1_REPORTS_DIR"
...
def add_repo_paths() -> None:
    ordered_paths = [str(MYDIFFUSION_D1_ROOT), str(REPO_ROOT)]
...
def module_install_hint(module_name: str) -> str:
    hints = {
        ...
        "robomimic": (
            ...
            "packages listed in `mydiffusion_D1/requirements_mimicgen.txt`."
        ),
        "robosuite": (
        ...
        "diffusers": "Install Hugging Face diffusers with `pip install diffusers`.",
        "einops": "Install einops with `pip install einops`.",
        "dill": "Install dill with `pip install dill`.",
        "av": "Install PyAV with `pip install av`.",
        "zarr": "Install zarr with `pip install zarr numcodecs`.",
        "numba": "Install numba with `pip install numba`.",
        "scipy": "Install SciPy with `pip install scipy`.",
    }
...
def require_modules(module_names: Sequence[str]) -> None:
    ...
    if missing:
        ...
        fail(
            ...
            f"{detail}\n"
            "Also review `mydiffusion_D1/requirements_mimicgen.txt` for the tested package set."
        )
...
def raw_dataset_root() -> Path:
    data_root = _path_from_env(MYDIFFUSION_D1_DATA_DIR_ENV_VAR, MYDIFFUSION_D1_ROOT / "data")
    return data_root / "mimicgen" / "raw"
...
def processed_dataset_path(task_name: str) -> Path:
    data_root = _path_from_env(MYDIFFUSION_D1_DATA_DIR_ENV_VAR, MYDIFFUSION_D1_ROOT / "data")
    return data_root / "mimicgen" / "processed" / task_name / "low_dim_abs.hdf5"


def reports_dir() -> Path:
    return _path_from_env(MYDIFFUSION_D1_REPORTS_DIR_ENV_VAR, MYDIFFUSION_D1_ROOT / "reports")


def outputs_dir() -> Path:
    return _path_from_env(MYDIFFUSION_D1_OUTPUTS_DIR_ENV_VAR, MYDIFFUSION_D1_ROOT / "outputs")
...
def config_root() -> Path:
    return DIFFUSION_POLICY_ROOT / "config"


def task_config_path(task_name: str) -> Path:
    return config_root() / "task" / f"{task_config_name(task_name)}.yaml"
...
def task_config_yaml(
    ...
    dataset_path = (
        "${oc.env:"
        f"{MYDIFFUSION_D1_DATA_DIR_ENV_VAR},mydiffusion_D1/data"
        f"}}/mimicgen/processed/{task_name}/low_dim_abs.hdf5"
    )
    obs_keys_literal = "[" + ", ".join(repr(key) for key in obs_keys) + "]"
    return f"""# Auto-generated by mydiffusion_D1/scripts/inspect_mimicgen_dataset.py
name: {config_name}
